...
{
  if ( T60 <= 0.0 ) {
    std::cout << "NRev::NRev: argument (" << T60 << ") must be positive!";
...
{
  if ( T60 <= 0.0 ) {
    std::cout << "NRev::setT60: argument (" << T60 << ") must be positive!";
...
StkFrames& NRev :: tick( StkFrames& frames, unsigned int channel )
{
  ...
  if ( channel >= frames.channels() - 1 ) {
     std::cout << "NRev::tick(): channel and StkFrames arguments are incompatible!";
    handleError( StkError::FUNCTION_ARGUMENT );
  }
#endif
